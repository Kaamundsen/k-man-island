...
export default function MarkedsskannerPage() {
    ...
    <main className="min-h-screen p-8">
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        <div className="bg-surface rounded-2xl p-4 border border-surface-border">
          ...
          <div className="text-3xl font-extrabold text-brand-slate">{mockStocks.length}</div>
        </div>
        <div className="bg-gradient-to-br from-brand-emerald to-emerald-600 rounded-2xl p-4 text-white">
          ...
          <div className="text-3xl font-extrabold">{mockStocks.filter(s => s.signal === 'BUY').length}</div>
        </div>
        <div className="bg-gradient-to-br from-gray-400 to-gray-600 rounded-2xl p-4 text-white">
          ...
          <div className="text-3xl font-extrabold">{mockStocks.filter(s => s.signal === 'HOLD').length}</div>
        </div>
        <div className="bg-gradient-to-br from-brand-rose to-rose-600 rounded-2xl p-4 text-white">
          ...
          <div className="text-3xl font-extrabold">{mockStocks.filter(s => s.signal === 'SELL').length}</div>
        </div>
      </div>
      ...
      <div className="mt-6 text-center text-sm text-gray-500">
        Viser {filteredAndSortedStocks.length} av {mockStocks.length} aksjer
      </div>
    </main>
  );
}
